...
GLint gMainWindow, gLeftWindow, gRightWindow;
...
int main(int argc, char* argv[])
{
  ...
  glutInitWindowSize(1200, 600);			//set window size in pixels
  glutInitWindowPosition(50, 50);       //set window position from top-left corner of display
  gMainWindow = glutCreateWindow("Kitchen");
  ...
  int gap = 15;
  int size = (1200 - gap*3)/2;
  
  gLeftWindow = glutCreateSubWindow(gMainWindow, gap, gap, size , size);
  init();
  ...
  gRightWindow = glutCreateSubWindow(gMainWindow, gap + size + gap, gap, size , size);
  init();
