...
void myKeypress(unsigned char key, int x, int y)
{
  ...
  switch(key)
  {
    ...
    case 'r':  
               gCameraPosition[0] = 0;
               gCameraPosition[1] = 10.0;
               gCameraPosition[2] = 23.0;
               gCameraLookAt[0] = 0;
               gCameraLookAt[1] = 10;
               gCameraLookAt[2] = 14;
               gCameraUp[0] = 0;
...
void myIdleFunc()
{
  ...
  if ( spotlightX == 5 )
     rightMax = true;
  else if ( spotlightX == -5 )
     leftMax = true;
  else if ( spotlightX > 0 && spotlightX <= 5 )
  {
     rightSide = true;
     leftSide = false;
     leftMax = false;
  ...
  else if ( spotlightX >= -5 && spotlightX < 0 )
  {
     leftSide = true;
     rightSide = false;
     rightMax = false;
...
/*
  if ( leftSide   )
       ...
       if ( passOthroughR )
          cout << "PASS O THROUGH R " << endl;
       else if ( passOthroughL )
            cout << "PASS O THROUGH L " << endl;
*/
  cout << "SPotLightX " << spotlightY << endl;
  //glutPostRedisplay();  //refresh display
  glutPostWindowRedisplay(subwindow_2);
}
